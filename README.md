...
**A RESTful API that extracts anime metadata and episode playback links from ANIMO**
...
- [Documentation](#documentation)
  ...
  - [Episode Player](#13-get-episode-player)
...
hianime-api is an unofficial RESTful API that retrieves anime metadata, episode lists, and external player links from [ANIMO](https://4animo.xyz/). It supports ANIMO's current Next.js data format while retaining legacy HTML extraction as a fallback.
...
2. This is an **unofficial API for [ANIMO](https://4animo.xyz/)** and is not affiliated with ANIMO or its content providers.
...
Episode responses include ready-to-use `embedUrl` and `dubEmbedUrl` values. You can also generate one through `/api/v2/player/:id/:episode`.

**Embed Example URL:** `https://cdn.4animo.xyz/embed/hd-1/12/1/sub?k=1`

The player URL follows this format:

```text
https://cdn.4animo.xyz/embed/{server}/{animeId}/{episodeNumber}/{type}?k=1
```

- `server`: `hd-1`, `hd-2`, `hd-3`, or `hd-4`
- `type`: `sub` or `dub`
- Check the episode's `sub` and `dub` fields before offering each language.
- Server availability can vary by anime or episode. If one server fails, try the next one.

For example, Solo Leveling Season 2 episode 1 has these player choices:

```text
SUB: hd-1, hd-2, hd-3, hd-4
DUB: hd-1, hd-2, hd-3, hd-4
```

You can construct a selected server URL directly:

```javascript
function getPlayerUrl(animeId, episodeNumber, type = 'sub', server = 'hd-1') {
  const servers = ['hd-1', 'hd-2', 'hd-3', 'hd-4'];
  if (!servers.includes(server)) throw new Error('Invalid server');
  if (!['sub', 'dub'].includes(type)) throw new Error('Invalid type');

  return `https://cdn.4animo.xyz/embed/${server}/${animeId}/${episodeNumber}/${type}?k=1`;
}

const playerUrl = getPlayerUrl(18120, 1, 'dub', 'hd-3');
```
...
```html
<iframe
  src="https://cdn.4animo.xyz/embed/hd-1/12/1/sub?k=1"
  width="100%"
  height="500"
  frameborder="0"
...
The server will be running at [http://localhost:5001](http://localhost:5001). Set `PORT` to use another port.
...
```bash
docker run -p 5001:5001 hianime-api
```
...
const resp = await fetch(
  '/api/v2/filter?type=tv&status=currently_airing&sort=score&genres=action,fantasy&page=1'
);
const data = await resp.json();
console.log(data);
```
    ...
    "episodes": [
      {
        ...
        "episodeNumber": 1,
        "id": "one-piece-12?ep=1",
        "animeId": 12,
        "embedId": "2142",
        "thumbnail": "https://api.kryzox.xyz/episode/1.jpg",
        "isFiller": false,
        "sub": true,
        "dub": true,
...
### 13. GET Episode Player

Generate an external player URL for an anime episode.

**Endpoint:**

```
GET /api/v2/player/:id/:episode?type=sub
```

`type` may be `sub` (default) or `dub`.

This endpoint returns the default `hd-1` player. To select another server, replace the
`hd-1` segment in `embedUrl` with `hd-2`, `hd-3`, or `hd-4`:

```javascript
const resp = await fetch('/api/v2/player/one-piece-12/1?type=sub');
const { data } = await resp.json();

const selectedServer = 'hd-3';
const embedUrl = data.embedUrl.replace('/embed/hd-1/', `/embed/${selectedServer}/`);
